...
- Added `manual-angular-stack-major-updates` for repositories that perform coordinated Angular, Quadrel, ngx-translate, and TypeScript major migrations manually.

### Changed

- All four default presets now group updates from all managers, dependency types, and update types into one pull request.
- Automerge defaults merge the grouped pull request only when every included update is eligible for automerge.
...
- Re-enabled platform-native automerge for routine Maven, Dockerfile, and Docker Compose updates.
- Documented platform-team onboarding, preset selection, configuration examples, and the possible multi-run behavior of Renovate-managed automerge.
