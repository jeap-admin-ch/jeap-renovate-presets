...
Update pull requests follow the selected presets. With the non-automerge defaults, the team reviews and merges every pull request. With an automerge default, Renovate merges eligible routine Maven, Dockerfile, and Docker Compose updates only after required checks and branch protection conditions pass. Major updates are never enabled for automerge by the provided `automerge` preset.
